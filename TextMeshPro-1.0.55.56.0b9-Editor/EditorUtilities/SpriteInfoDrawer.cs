...
namespace TMPro.EditorUtilities
{
	...
	public class SpriteInfoDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			{
				vector2.y = serializedProperty7.floatValue * vector2.x / serializedProperty6.floatValue;
				vector.y += (vector2.x - vector2.y) / 2f;
			}
			else
			{
				vector2.x = serializedProperty6.floatValue * vector2.y / serializedProperty7.floatValue;
				vector.x += (vector2.y - vector2.x) / 2f;
			}
			Rect texCoords = new Rect(serializedProperty4.floatValue / (float)spriteSheet.width, serializedProperty5.floatValue / (float)spriteSheet.height, serializedProperty6.floatValue / (float)spriteSheet.width, serializedProperty7.floatValue / (float)spriteSheet.height);
			GUI.DrawTextureWithTexCoords(new Rect(vector.x + 5f, vector.y + 2.5f, vector2.x, vector2.y), spriteSheet, texCoords, true);
