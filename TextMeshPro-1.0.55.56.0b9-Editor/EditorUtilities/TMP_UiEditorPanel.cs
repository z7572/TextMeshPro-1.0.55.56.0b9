...
namespace TMPro.EditorUtilities
{
	...
	public class TMP_UiEditorPanel : Editor
	{
		public void OnEnable()
		{
			...
			this.raycastTarget_prop = base.serializedObject.FindProperty("m_RaycastTarget");
			...
			this.m_textComponent = (base.target as TextMeshProUGUI);
		...
		public override void OnInspectorGUI()
		{
			...
			controlRect.y += 2f;
			GUI.Label(controlRect, "<b>TEXT INPUT BOX</b>" + ((!TMP_UiEditorPanel.m_foldout.textInput) ? TMP_UiEditorPanel.uiStateLabel[0] : TMP_UiEditorPanel.uiStateLabel[1]), TMP_UIStyleManager.Section_Label);
			...
			{
				TMP_UiEditorPanel.m_foldout.textInput = !TMP_UiEditorPanel.m_foldout.textInput;
			}
			...
			if (TMP_UiEditorPanel.m_foldout.textInput)
			{
			...
			if (GUILayout.Button("<b>FONT SETTINGS</b>" + ((!TMP_UiEditorPanel.m_foldout.fontSettings) ? TMP_UiEditorPanel.uiStateLabel[0] : TMP_UiEditorPanel.uiStateLabel[1]), TMP_UIStyleManager.Section_Label, new GUILayoutOption[0]))
			{
				TMP_UiEditorPanel.m_foldout.fontSettings = !TMP_UiEditorPanel.m_foldout.fontSettings;
			}
			if (this.m_isPresetListDirty)
			{
				this.m_materialPresetNames = this.GetMaterialPresets();
			}
			if (TMP_UiEditorPanel.m_foldout.fontSettings)
			{
				EditorGUI.BeginChangeCheck();
				...
				{
					EditorGUI.BeginChangeCheck();
					controlRect = EditorGUILayout.GetControlRect(false, 17f, new GUILayoutOption[0]);
			...
			if (GUILayout.Button("<b>EXTRA SETTINGS</b>" + ((!TMP_UiEditorPanel.m_foldout.extraSettings) ? TMP_UiEditorPanel.uiStateLabel[0] : TMP_UiEditorPanel.uiStateLabel[1]), TMP_UIStyleManager.Section_Label, new GUILayoutOption[0]))
			{
				TMP_UiEditorPanel.m_foldout.extraSettings = !TMP_UiEditorPanel.m_foldout.extraSettings;
			}
			if (TMP_UiEditorPanel.m_foldout.extraSettings)
			{
				...
				EditorGUI.BeginChangeCheck();
				EditorGUILayout.PropertyField(this.raycastTarget_prop, new GUIContent("Raycast Target?"), new GUILayoutOption[0]);
				if (EditorGUI.EndChangeCheck())
				{
					Graphic[] componentsInChildren = this.m_textComponent.GetComponentsInChildren<Graphic>();
					for (int k = 0; k < componentsInChildren.Length; k++)
					{
						componentsInChildren[k].raycastTarget = this.raycastTarget_prop.boolValue;
					}
					this.havePropertiesChanged = true;
				}
				EditorGUILayout.EndHorizontal();
		...
		public void OnSceneGUI()
		{
			...
			if (TMP_UiEditorPanel.cache0 == null)
			{
				TMP_UiEditorPanel.cache0 = new Handles.CapFunction(Handles.DotHandleCap);
			}
			Vector3 rhs = Handles.FreeMoveHandle(position, identity, size, zero, TMP_UiEditorPanel.cache0);
			bool flag = false;
			if (vector != rhs)
			{
			...
			if (TMP_UiEditorPanel.cache1 == null)
			{
				TMP_UiEditorPanel.cache1 = new Handles.CapFunction(Handles.DotHandleCap);
			}
			Vector3 rhs2 = Handles.FreeMoveHandle(position2, identity2, size2, zero2, TMP_UiEditorPanel.cache1);
			if (vector2 != rhs2)
			{
			...
			if (TMP_UiEditorPanel.cache2 == null)
			{
				TMP_UiEditorPanel.cache2 = new Handles.CapFunction(Handles.DotHandleCap);
			}
			Vector3 rhs3 = Handles.FreeMoveHandle(position3, identity3, size3, zero3, TMP_UiEditorPanel.cache2);
			if (vector3 != rhs3)
			{
			...
			if (TMP_UiEditorPanel.cache3 == null)
			{
				TMP_UiEditorPanel.cache3 = new Handles.CapFunction(Handles.DotHandleCap);
			}
			Vector3 rhs4 = Handles.FreeMoveHandle(position4, identity4, size4, zero4, TMP_UiEditorPanel.cache3);
			if (vector4 != rhs4)
			{
		...
		private void DrawPropertyBlock(string[] labels, SerializedProperty[] properties)
		{
			...
			for (int i = 0; i < labels.Length; i++)
			{
				if (i == 0)
				{
					...
					EditorGUI.PropertyField(new Rect(controlRect.x - 20f, controlRect.y, 75f, controlRect.height), properties[i], new GUIContent("  "));
					controlRect.x += controlRect.width;
				}
				else
				{
		...
		private bool IsMixSelectionTypes()
		{
			...
			if (gameObjects.Length > 1)
			{
				...
				{
					if (((GameObject)gameObjects[i]).GetComponent<TextMeshProUGUI>() == null)
					{
						return true;
					}
				}
