	...
	public class TextMeshPro : TMP_Text, ILayoutElement
	{
		...
		protected override void OnEnable()
		{
			if (!this.m_isRegisteredForEvents)
			{
				...
				TMPro_EventManager.TEXTMESHPRO_PROPERTY_EVENT.Add(new Action<bool, TextMeshPro>(this.ON_TEXTMESHPRO_PROPERTY_CHANGED));
		...
		protected override void OnDestroy()
		{
			...
			TMPro_EventManager.TEXTMESHPRO_PROPERTY_EVENT.Remove(new Action<bool, TextMeshPro>(this.ON_TEXTMESHPRO_PROPERTY_CHANGED));
		...
		{
			if (this.m_renderer.sharedMaterial == null)
			{
				if (this.m_fontAsset != null)
				{
					...
					{
						"No Material was assigned to ",
						base.name,
						". ",
						this.m_fontAsset.material.name,
						" was assigned."
					}), this);
				}
				else
			...
			if (this.m_renderer.sharedMaterial != this.m_sharedMaterial)
			{
		...
		private void ON_DRAG_AND_DROP_MATERIAL(GameObject obj, Material currentMaterial, Material newMaterial)
		{
			if (obj == base.gameObject || PrefabUtility.GetPrefabParent(base.gameObject) == obj)
			{
				...
				Undo.RecordObject(this.m_renderer, "Material Assignment");
		...
		protected override void GenerateTextMesh()
		{
			...
			while (num14 < this.m_char_buffer.Length && this.m_char_buffer[num14] != 0)
			{
				...
				else if (!this.m_isRightToLeft)
				{
					...
					if (this.m_isFXMatrixSet)
					{
						num35 = this.m_FXMatrix.m00;
					}
					this.m_xAdvance += ((this.m_cached_TextElement.xAdvance * num35 * num4 + this.m_characterSpacing + this.m_currentFontAsset.normalSpacingOffset) * num2 + this.m_cSpacing) * (1f - this.m_charWidthAdjDelta);
					if (char.IsWhiteSpace((char)num15) || num15 == 8203)
					{
						this.m_xAdvance += this.m_wordSpacing * num2;
					}
				...
				if (num15 == 10 || this.m_characterCount == totalCharacterCount - 1)
				{
					if (this.m_lineNumber > 0 && !TMP_Math.Approximately(this.m_maxLineAscender, this.m_startOfLineAscender) && this.m_lineHeight == -32767f && !this.m_isNewPage)
					{
						...
						this.AdjustLineOffset(this.m_firstCharacterOfLine, this.m_characterCount, num36);
						num24 -= num36;
						this.m_lineOffset += num36;
					}
					this.m_isNewPage = false;
					float num37 = this.m_maxLineAscender - this.m_lineOffset;
					float num38 = this.m_maxLineDescender - this.m_lineOffset;
					this.m_maxDescender = ((this.m_maxDescender >= num38) ? num38 : this.m_maxDescender);
					if (!flag5)
					{
						num11 = this.m_maxDescender;
					}
					...
					this.m_textInfo.lineInfo[this.m_lineNumber].characterCount = this.m_textInfo.lineInfo[this.m_lineNumber].lastCharacterIndex - this.m_textInfo.lineInfo[this.m_lineNumber].firstCharacterIndex + 1;
					this.m_textInfo.lineInfo[this.m_lineNumber].visibleCharacterCount = this.m_lineVisibleCharacterCount;
					this.m_textInfo.lineInfo[this.m_lineNumber].lineExtents.min = new Vector2(this.m_textInfo.characterInfo[this.m_firstVisibleCharacterOfLine].bottomLeft.x, num38);
					this.m_textInfo.lineInfo[this.m_lineNumber].lineExtents.max = new Vector2(this.m_textInfo.characterInfo[this.m_lastVisibleCharacterOfLine].topRight.x, num37);
					...
					this.m_textInfo.lineInfo[this.m_lineNumber].baseline = 0f - this.m_lineOffset;
					this.m_textInfo.lineInfo[this.m_lineNumber].ascender = num37;
					this.m_textInfo.lineInfo[this.m_lineNumber].descender = num38;
					this.m_textInfo.lineInfo[this.m_lineNumber].lineHeight = num37 - num38 + num5 * num;
			...
			float num39 = this.m_maxFontSize - this.m_minFontSize;
			if (!this.m_isCharacterWrappingEnabled && this.m_enableAutoSizing && num39 > 0.051f && this.m_fontSize < this.m_fontSizeMax)
			{
			...
			else
			{
				...
				int num40 = this.m_materialReferences[0].referenceCount * (this.m_isVolumetricText ? 8 : 4);
				...
				while (i < this.m_characterCount)
				{
					...
					if (isVisible)
					{
						...
						else
						{
							...
							case TextureMappingOptions.Line:
								...
								{
									characterInfo[i].vertex_BL.uv2.x = (characterInfo[i].vertex_BL.position.x + vector6.x - this.m_meshExtents.min.x) / (this.m_meshExtents.max.x - this.m_meshExtents.min.x) + num52;
									characterInfo[i].vertex_TL.uv2.x = (characterInfo[i].vertex_TL.position.x + vector6.x - this.m_meshExtents.min.x) / (this.m_meshExtents.max.x - this.m_meshExtents.min.x) + num52;
									characterInfo[i].vertex_TR.uv2.x = (characterInfo[i].vertex_TR.position.x + vector6.x - this.m_meshExtents.min.x) / (this.m_meshExtents.max.x - this.m_meshExtents.min.x) + num52;
									characterInfo[i].vertex_BR.uv2.x = (characterInfo[i].vertex_BR.position.x + vector6.x - this.m_meshExtents.min.x) / (this.m_meshExtents.max.x - this.m_meshExtents.min.x) + num52;
								}
								break;
						...
						if (i < this.m_maxVisibleCharacters && num41 < this.m_maxVisibleWords && lineNumber3 < this.m_maxVisibleLines && this.m_overflowMode != TextOverflowModes.Page)
						{
							TMP_CharacterInfo[] array = characterInfo;
							int num61 = i;
							array[num61].vertex_BL.position = array[num61].vertex_BL.position + b4;
							TMP_CharacterInfo[] array2 = characterInfo;
							int num62 = i;
							array2[num62].vertex_TL.position = array2[num62].vertex_TL.position + b4;
							TMP_CharacterInfo[] array3 = characterInfo;
							int num63 = i;
							array3[num63].vertex_TR.position = array3[num63].vertex_TR.position + b4;
							TMP_CharacterInfo[] array4 = characterInfo;
							int num64 = i;
							array4[num64].vertex_BR.position = array4[num64].vertex_BR.position + b4;
						}
						else if (i < this.m_maxVisibleCharacters && num41 < this.m_maxVisibleWords && lineNumber3 < this.m_maxVisibleLines && this.m_overflowMode == TextOverflowModes.Page && characterInfo[i].pageNumber == num6)
						{
							TMP_CharacterInfo[] array5 = characterInfo;
							int num65 = i;
							array5[num65].vertex_BL.position = array5[num65].vertex_BL.position + b4;
							TMP_CharacterInfo[] array6 = characterInfo;
							int num66 = i;
							array6[num66].vertex_TL.position = array6[num66].vertex_TL.position + b4;
							TMP_CharacterInfo[] array7 = characterInfo;
							int num67 = i;
							array7[num67].vertex_TR.position = array7[num67].vertex_TR.position + b4;
							TMP_CharacterInfo[] array8 = characterInfo;
							int num68 = i;
							array8[num68].vertex_BR.position = array8[num68].vertex_BR.position + b4;
						}
						else
						{
					...
					int num69 = i;
					characterInfo2[num69].bottomLeft = characterInfo2[num69].bottomLeft + b4;
					TMP_CharacterInfo[] characterInfo3 = this.m_textInfo.characterInfo;
					int num70 = i;
					characterInfo3[num70].topLeft = characterInfo3[num70].topLeft + b4;
					TMP_CharacterInfo[] characterInfo4 = this.m_textInfo.characterInfo;
					int num71 = i;
					characterInfo4[num71].topRight = characterInfo4[num71].topRight + b4;
					TMP_CharacterInfo[] characterInfo5 = this.m_textInfo.characterInfo;
					int num72 = i;
					characterInfo5[num72].bottomRight = characterInfo5[num72].bottomRight + b4;
					TMP_CharacterInfo[] characterInfo6 = this.m_textInfo.characterInfo;
					int num73 = i;
					characterInfo6[num73].origin = characterInfo6[num73].origin + b4.x;
					TMP_CharacterInfo[] characterInfo7 = this.m_textInfo.characterInfo;
					int num74 = i;
					characterInfo7[num74].xAdvance = characterInfo7[num74].xAdvance + b4.x;
					TMP_CharacterInfo[] characterInfo8 = this.m_textInfo.characterInfo;
					int num75 = i;
					characterInfo8[num75].ascender = characterInfo8[num75].ascender + b4.y;
					TMP_CharacterInfo[] characterInfo9 = this.m_textInfo.characterInfo;
					int num76 = i;
					characterInfo9[num76].descender = characterInfo9[num76].descender + b4.y;
					TMP_CharacterInfo[] characterInfo10 = this.m_textInfo.characterInfo;
					int num77 = i;
					characterInfo10[num77].baseLine = characterInfo10[num77].baseLine + b4.y;
					if (isVisible)
					{
					}
					if (lineNumber3 != num42 || i == this.m_characterCount - 1)
					{
						if (lineNumber3 != num42)
						{
							...
							int num78 = num42;
							lineInfo3[num78].baseline = lineInfo3[num78].baseline + b4.y;
							TMP_LineInfo[] lineInfo4 = this.m_textInfo.lineInfo;
							int num79 = num42;
							lineInfo4[num79].ascender = lineInfo4[num79].ascender + b4.y;
							TMP_LineInfo[] lineInfo5 = this.m_textInfo.lineInfo;
							int num80 = num42;
							lineInfo5[num80].descender = lineInfo5[num80].descender + b4.y;
						...
						if (i == this.m_characterCount - 1)
						{
							...
							lineInfo6[num81].baseline = lineInfo6[num81].baseline + b4.y;
							TMP_LineInfo[] lineInfo7 = this.m_textInfo.lineInfo;
							int num82 = lineNumber3;
							lineInfo7[num82].ascender = lineInfo7[num82].ascender + b4.y;
							TMP_LineInfo[] lineInfo8 = this.m_textInfo.lineInfo;
							int num83 = lineNumber3;
							lineInfo8[num83].descender = lineInfo8[num83].descender + b4.y;
					...
					else if (flag11 || (i == 0 && (!char.IsPunctuation(character2) || char.IsWhiteSpace(character2) || character2 == '​' || i == this.m_characterCount - 1)))
					{
						...
						{
							int num85 = (i != this.m_characterCount - 1 || !char.IsLetterOrDigit(character2)) ? (i - 1) : i;
							flag11 = false;
					...
					if (flag12)
					{
						...
						if (!char.IsWhiteSpace(character2) && character2 != '​')
						{
							...
							num48 = Mathf.Min((pageNumber != num49) ? TMP_Text.k_LargePositiveFloat : num48, this.m_textInfo.characterInfo[i].baseLine + base.font.fontInfo.Underline * num47);
							num49 = pageNumber;
						}
						...
						if (flag && this.m_characterCount == 1)
						{
							...
							this.DrawUnderlineMesh(zero, zero2, ref num40, num46, scale, num47, num45, color);
							num47 = 0f;
					...
					if (flag14)
					{
						...
						if (!flag2 && flag15 && i <= tmp_LineInfo.lastVisibleCharacterIndex && character2 != '\n' && character2 != '\r')
						{
							if (i != tmp_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character2))
							{
								flag2 = true;
						...
						if (flag2 && this.m_characterCount == 1)
						{
							...
							this.DrawUnderlineMesh(zero3, zero4, ref num40, num51, num51, num51, num45, underlineColor);
						}
						...
						else if (flag2 && i < this.m_characterCount && (this.m_textInfo.characterInfo[i + 1].pointSize != num50 || !TMP_Math.Approximately(this.m_textInfo.characterInfo[i + 1].baseLine + b4.y, b5)))
						{
							flag2 = false;
					...
					if (flag16)
					{
						...
						if (!flag3 && flag17 && i <= tmp_LineInfo.lastVisibleCharacterIndex && character2 != '\n' && character2 != '\r')
						{
							if (i != tmp_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character2))
							{
								flag3 = true;
						...
						if (flag3)
						{
							...
							if (!highlightColor.Compare(highlightColor2))
							{
								...
								this.DrawTextHighlight(start, vector, ref num40, highlightColor);
								flag3 = true;
								start = vector;
					...
					if ((!char.IsControl(character3) && lineNumber3 < this.m_lineNumber) || flag19 || tmp_LineInfo.maxAdvance > tmp_LineInfo.width)
					{
						...
						else
						{
							...
							int num91 = (!characterInfo[tmp_LineInfo.lastCharacterIndex].isVisible) ? (tmp_LineInfo.spaceCount - 1) : tmp_LineInfo.spaceCount;
							if (flag10)
							{
								num91--;
								num90++;
							}
							float num92 = (num91 <= 0) ? 1f : this.m_wordWrappingRatios;
							if (num91 < 1)
							{
								num91 = 1;
							}
							if (character2 == '\t' || char.IsSeparator(character2))
							{
								if (!this.m_isRightToLeft)
								{
									vector6 += new Vector3(num89 * (1f - num92) / (float)num91, 0f, 0f);
								}
								else
								{
									vector6 -= new Vector3(num89 * (1f - num92) / (float)num91, 0f, 0f);
							...
							{
								vector6 += new Vector3(num89 * num92 / (float)num90, 0f, 0f);
							}
							else
							{
								vector6 -= new Vector3(num89 * num92 / (float)num90, 0f, 0f);
				...
				this.m_textInfo.wordCount = ((num41 == 0 || this.m_characterCount <= 0) ? 1 : num41);
