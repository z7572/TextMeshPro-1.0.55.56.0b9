	...
	public class TMP_SubMeshUI : MaskableGraphic, IClippable, IMaskable, IMaterialModifier
	{
		...
		protected override void OnEnable()
		{
			if (!this.m_isRegisteredForEvents)
			{
				...
				TMPro_EventManager.SPRITE_ASSET_PROPERTY_EVENT.Add(new Action<bool, UnityEngine.Object>(this.ON_SPRITE_ASSET_PROPERTY_CHANGED));
		...
		protected override void OnDestroy()
		{
			...
			TMPro_EventManager.SPRITE_ASSET_PROPERTY_EVENT.Remove(new Action<bool, UnityEngine.Object>(this.ON_SPRITE_ASSET_PROPERTY_CHANGED));
		...
		private void ON_MATERIAL_PROPERTY_CHANGED(bool isChanged, Material mat)
		{
			...
			int num2 = (!(this.m_fallbackSourceMaterial == null)) ? this.m_fallbackSourceMaterial.GetInstanceID() : 0;
			if (this.m_fallbackMaterial != null && num2 == instanceID)
			{
				TMP_MaterialManager.CopyMaterialPresetProperties(mat, this.m_fallbackMaterial);
			}
			...
			if (this.m_MaskMaterial != null)
			{
				...
				{
					float float3 = this.m_MaskMaterial.GetFloat(ShaderUtilities.ID_StencilID);
					float float4 = this.m_MaskMaterial.GetFloat(ShaderUtilities.ID_StencilComp);
					this.m_MaskMaterial.CopyPropertiesFromMaterial(this.m_fallbackMaterial);
					this.m_MaskMaterial.shaderKeywords = this.m_fallbackMaterial.shaderKeywords;
					this.m_MaskMaterial.SetFloat(ShaderUtilities.ID_StencilID, float3);
					this.m_MaskMaterial.SetFloat(ShaderUtilities.ID_StencilComp, float4);
				}
			}
			this.m_padding = this.GetPaddingForMaterial();
			this.SetVerticesDirty();
		...
		private void ON_DRAG_AND_DROP_MATERIAL(GameObject obj, Material currentMaterial, Material newMaterial)
		{
			if (obj == base.gameObject || PrefabUtility.GetPrefabParent(base.gameObject) == obj)
			{
				...
				this.SetSharedMaterial(newMaterial);
		...
		{
			if (font.GetInstanceID() == this.m_fontAsset.GetInstanceID() && this.m_fallbackMaterial != null)
			{
				this.m_fallbackMaterial.SetFloat(ShaderUtilities.ID_WeightNormal, this.m_fontAsset.normalStyle);
				this.m_fallbackMaterial.SetFloat(ShaderUtilities.ID_WeightBold, this.m_fontAsset.boldStyle);
			}
		}
		...
		protected override void UpdateMaterial()
		{
			...
			if (this.m_sharedMaterial != null && base.gameObject.name != "TMP SubMeshUI [" + this.m_sharedMaterial.name + "]")
			{
				base.gameObject.name = "TMP SubMeshUI [" + this.m_sharedMaterial.name + "]";
			}
		}
